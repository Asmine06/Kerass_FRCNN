...
def get_noise_data (input_path , name_data, per_Noise_add_bbox):
        #pdb.set_trace()
        all_imgs = []
                ...
                try:
                    ...
                    if name_data == 'train' and  annotation_data['imageset'] == 'train':
                        ...
                        for element_obj in element_objs:
                            ...
                            rNoise_add_bbox= random.random()

                            if rNoise_add_bbox <= per_Noise_add_bbox:
                                ...
                                print(element_width)
                                #x1 = int(round(float(obj_bbox.find('xmin').text)))
                                x1=random.uniform(0,width-0.5*width)
                                #y1 = int(round(float(obj_bbox.find('ymin').text)))
                                y1=random.uniform(0,height-0.5*height)
                                #x2 = int(roundd(float(obj_bbox.find('xmax').text)))
                                x2=x1+random.uniform(0,0.5)*width
                                #y2 = int(round(float(obj_bbox.find('ymax').text)))
                                y2=y1+random.uniform(0,0.5)*height
                                difficulty = int(element_obj.find('difficult').text) == 1
